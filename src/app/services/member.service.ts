...
export class MemberService {
  ...
  getMember(cust_id: number, include_licenses?: boolean): Promise<Member> {
    return this.getMemberList([cust_id], include_licenses).then(members => members[0]);
  }
  ...
  getMemberList(cust_ids: number[], include_licenses?: boolean): Promise<Member[]> {
    ...
    return fetch(`${this.baseUrl}/get?cust_ids=${cust_ids_strings}${include_licenses_param}`)
    .then(r => r.json())
    .then(r => r['members']);
  }
